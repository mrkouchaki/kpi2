...
func (c Control) Consume(msg *xapp.RMRParams) error {
	...
	fmt.Println("/////////in func c Control Consume")
	xapp.Logger.Info(
...
func create_db() {
	...
	fmt.Println("/////////in func NewControl Control create_db named kpimon in influxDB is done")
	if err != nil {
		xapp.Logger.Error("Create database failed!")
		fmt.Println("/////////in func NewControl Control create_db process Create database has been failed line 81")
	}
}
...
func (c *Control) getnbList() []*xapp.RNIBNbIdentity {
	...
	if enbs, err := c.getEnbList(); err == nil {
		nbs = append(nbs, enbs...)
		fmt.Println("///////// in getnbList fun enbs =", nbs)

	}

	if gnbs, err := c.getGnbList(); err == nil {
		nbs = append(nbs, gnbs...)
		fmt.Println("///////// in getnb if there is not error")
...
func (c Control) sendSubscription(meid string) {
	...
	fmt.Println("//////////in sendsubscription func Create Subscription message and send it to RIC platform")
	xapp.Logger.Info("Sending subscription request for MEID: %v", meid)
	fmt.Println("//////////in sendsubscription func ,meid=" , meid)
	...
	fmt.Println("/////////resp in func (c Control) sendSubscription resp= ", resp)
	fmt.Println("/////////err= func (c Control) sendSubscription err=", err)
	if err != nil {
		fmt.Println("////////////////////first if in func (c Control) sendSubscription enter in err")
	...
	fmt.Println("/////Successfully subscription passed")
}

func (c *Control) controlLoop() {
	//Handle receiving message based on message type
	fmt.Println("//////////entered func ( c ontrol) controlLoop ")
	for {
	        fmt.Println("//////////entered for loop in controlloop func")
		msg := <-c.RMR
		fmt.Println("//////////in fun Control controlLoop msg mines RMR, msg=",msg)
		fmt.Println("////passed msg in for loop and wants to go to switch mtype")
		xapp.Logger.Debug("Received message type: %d", msg.Mtype)
		switch msg.Mtype {
		case xapp.RIC_INDICATION:
			fmt.Println("/////in case xapp ric indication want to go into handleindication")
			go c.handleIndication(msg)
			fmt.Println("/////come back from handleindication")
		default:
			xapp.Logger.Error("Unknown Message Type '%d', discarding", msg.Mtype)
			fmt.Println("//////////on func c Unknown Message Type discarding msg Mtype=",msg.Mtype)
		}
	}
}
func (c *Control) handleIndication(params *xapp.RMRParams) (err error) {
	fmt.Println("//////////entered handleIndication")
	var e2ap *E2ap
	var e2sm *E2sm

	//Decode message and put it into log
	fmt.Println("//////////entered func c Control handleIndication")
	indicationMsg, err := e2ap.GetIndicationMessage(params.Payload)
	if err != nil {
	        fmt.Println("//////////entered first if err != nil in func c Control handleIndication")
	...
	fmt.Println("//////RIC indication message",params.Meid.RanName)
	log.Printf("RequestID: %d", indicationMsg.RequestID)
	fmt.Println("//////RIC indication message",indicationMsg.RequestID)
	log.Printf("RequestSequenceNumber: %d", indicationMsg.RequestSequenceNumber)
	fmt.Println("//////RIC indication message",indicationMsg.RequestSequenceNumber)
	...
	fmt.Println("/////////indicationHdr in func c Control handleIndication= ", indicationHdr)
	fmt.Println("/////////err= func c Control handleIndication", err)
	if err != nil {
	        fmt.Println("//////////entered second if err != nil in func c Control handleIndication")
	...
	if indicationHdr.IndHdrType == 1 {
		...
		if indHdrFormat1.GlobalKPMnodeIDType == 1 {
			...
			fmt.Println("//////////entered second if of Decoding Ric Indication Header handleIndication globalgNBID=",globalgNBID)
			...
			if globalgNBID.GnbIDType == 1 {
				...
				fmt.Println("//////////entered third if of Decoding Ric Indication Header handleIndication gNBID=",gNBID)
			...
			if globalKPMnodegNBID.GnbCUUPID != nil {
				...
				fmt.Println("//////////entered 4th if of IC Indication Header=",globalKPMnodegNBID.GnbCUUPID.Buf)
			}
			...
			if globalKPMnodeeNBID.EnbIDType == 1 {
				...
				log.Printf("eNB ID ID: %x, Unused: %d", eNBID.Buf, eNBID.BitsUnused)
				fmt.Println("//////////end else if Decoding Ric Indication Header")
		...
		if indHdrFormat1.NRCGI != nil {
			...
			fmt.Println("//////cellIDHdr", cellIDHdr)
			fmt.Println("/////err", err)
			if err != nil {
		...
		if indHdrFormat1.SliceID != nil {
			...
			if err != nil {
				...
				fmt.Println("Failed to parse SliceID in RIC Indication Header")
				return
			}
		} else {
			sliceIDHdr = -1
		...
		if indHdrFormat1.FiveQI != -1 {
			fmt.Println("///////indHdrFormat1.FiveQI not 1")
		...
		fmt.Println("Unknown RIC Indication Header Format indicationHdr.IndHdrType=",indicationHdr.IndHdrType)
		return
	}
	...
	if indMsg.IndMsgType == 1 {
		...
		fmt.Println("RIC Indication Message Format", indMsg.IndMsgType)
		...
		for PMContainerCounter := 0; PMContainerCounter < indMsgFormat1.PMContainerCount; PMContainerCounter++ {
			...
			if pmContainer.PFContainer != nil {
				...
				if containerType == 1 {
					...
					for cellResourceReportCounter := 0; cellResourceReportCounter < cellResourceReportCount; cellResourceReportCounter++ {
						...
						fmt.Println("TotalofAvailablePRBsDL", cellResourceReport.TotalofAvailablePRBs.DL)
						...
						if flag {
							...
							fmt.Println("////availPRBUL=",availPRBUL)
						}
						...
						for servedPlmnPerCellCounter := 0; servedPlmnPerCellCounter < servedPlmnPerCellCount; servedPlmnPerCellCounter++ {
							...
							if servedPlmnPerCell.DUPMEPC != nil {
								...
								for perQCIReportCounter := 0; perQCIReportCounter < perQCIReportCount; perQCIReportCounter++ {
									...
									fmt.Println("QCI:", perQCIReport.QCI)
									log.Printf("PrbUsageDL: %d", perQCIReport.PrbUsage.DL)
									fmt.Println("PrbUsageDL:", perQCIReport.PrbUsage.DL)
				...
				if containerType == 1 {
					...
					for CellResourceReportItemCounter := 0; CellResourceReportItemCounter < oDUUE.CellResourceReportItemCount; CellResourceReportItemCounter++ {
						...
						for UeResourceReportItemCounter := 0; UeResourceReportItemCounter < cellResourceReportItem.UeResourceReportItemCount; UeResourceReportItemCounter++ {
							...
							fmt.Println("UeID:", ueMetrics.UeID)
							...
							fmt.Println("ueMetrics=", ueMetrics)
			...
			if flag {
				...
				fmt.Println("cellMetrics=", cellMetrics)
				c.writeCellMetrics_db(cellMetrics)
			}
		}
	} else {
		fmt.Println("////else in Unknown RIC Indication")
...
func (c *Control) writeUeMetrics_db(ueMetrics UeMetricsEntry) {
	...
	fmt.Println("///// in Influx writeuemetric function ueMetricsJSON, := json.Marshal ueMetrics", ueMetricsJSON)
	fmt.Println("///// err := json.Marshal ueMetrics", err)
	if err != nil {
...
func (c *Control) writeCellMetrics_db(cellMetrics CellMetricsEntry) {
	...
	fmt.Println("//////////// in Influx writecellmetric function cellMetricsJSON, er := json.Marshal cellMetrics", cellMetrics)
        fmt.Println("////////// err in cellMetricsJSON", er)
	if er != nil {
...
func (c Control) xAppStartCB(d interface{}) {
	fmt.Println("//////////in xapp start func")
	...
	for _, nb := range nbList {
		fmt.Println("////in xapp start func: nb.ConnectionStatus=", nb.ConnectionStatus)
	...
	fmt.Println("//////////in start passed sensubscription want to go to controlloop")
